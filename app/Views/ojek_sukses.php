...
<head>
    ...
    <title>E-Voucher Ojek Sukses - Pendaki.id</title>
    ...
    <div class="w-full max-w-md md:max-w-4xl bg-white rounded-[2.5rem] shadow-2xl border border-gray-100 overflow-hidden relative flex flex-col md:flex-row my-6 md:my-10 transition-all duration-300">
        ...
        <div class="w-full md:w-1/2 flex flex-col border-b md:border-b-0 md:border-r border-gray-50">
            ...
            <div class="bg-gradient-to-br from-forest to-green-800 text-white p-8 text-center space-y-4 flex flex-col justify-center items-center md:h-2/5 flex-none">
                ...
                <div class="space-y-1">
                    <span class="text-[9px] font-black uppercase tracking-widest text-green-300 block">Pemesanan Ojek Berhasil</span>
                ...
                <div class="bg-black/20 rounded-full inline-block px-4 py-1.5 text-xs font-bold shadow-sm">
                    Kode Voucher: <span class="text-yellow-300"><?= esc($ticket_code) ?></span>
                </div>
            </div>
            
            <!-- Barcode & QR Code Section -->
            <div class="p-8 flex flex-col items-center justify-center text-center space-y-4 bg-white flex-1">
                <span class="text-xs text-gray-400 font-bold uppercase tracking-wider block">Pindai QR Code di Lokasi Basecamp</span>
                ...
                <p class="text-[10px] text-gray-400 font-semibold italic max-w-xs mx-auto">Tunjukkan barcode manifest ojek ini kepada koordinator ojek di loket pendaftaran basecamp untuk pembagian rombongan motor.</p>
            </div>
        </div>
        ...
        <div class="w-full md:w-1/2 bg-gray-50 p-8 flex flex-col justify-between flex-1">
            <div class="space-y-6">
                ...
                <div class="hidden md:block">
                    <h3 class="text-sm font-black text-gray-400 uppercase tracking-wider">Detail Pemesanan Ojek</h3>
                ...
                <div class="bg-white p-5 rounded-2xl border border-gray-100 text-xs space-y-4 shadow-sm">
                    <div class="flex justify-between items-center">
                        <span class="text-gray-400 font-bold uppercase tracking-wider">Team Leader</span>
                        <span class="font-extrabold text-gray-800 text-right"><?= esc($pesanan['NAMA_PENANGGUNG_JAWAB']) ?></span>
                    </div>
                    <div class="flex justify-between items-center">
                    ...
                    <div class="flex justify-between items-center">
                        <span class="text-gray-400 font-bold uppercase tracking-wider">Rute Perjalanan</span>
                        <span class="font-extrabold text-gray-800 text-right"><?= esc($pesanan['TITIK_JEMPUT']) ?> <i class="fa-solid fa-arrow-right text-[10px] mx-1 text-slate-400"></i> <?= esc($pesanan['POS_TUJUAN']) ?></span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-gray-400 font-bold uppercase tracking-wider">Tanggal Keberangkatan</span>
                        <span class="font-extrabold text-gray-800 text-right"><?= date('d F Y', strtotime($pesanan['TGL_KEBERANGKATAN'])) ?></span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-gray-400 font-bold uppercase tracking-wider">Jumlah Motor / Penumpang</span>
                        <span class="font-extrabold text-gray-800 text-right"><?= esc($pesanan['JML_PENUMPANG']) ?> Motor</span>
                    </div>

                    <div class="flex justify-between items-center pt-3 border-t border-gray-200">
                        <span class="text-gray-400 font-bold uppercase tracking-wider">Total Pembayaran</span>
                        <span class="font-black text-slate-900 text-sm">Rp <?= number_format($pesanan['TOT_BAYAR'], 0, ',', '.') ?></span>
                    </div>
